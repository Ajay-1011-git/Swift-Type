...
function TypingOptions() {
  ...
  return <div className="w-full flex justify-center mb-2">
  ...
  />
    ...
    /* Sub-options */
        ...
        {mode === "time" && <>
            {[15, 30, 60, 120].map((t) => <OptBtn key={t} active={timeLimit === t} onClick={() => handleTimeSelect(t)} label={`${t}`} />)}
          </>}
        {mode === "words" && <>
            {[10, 25, 50, 100].map((w) => <OptBtn key={w} active={wordLimit === w} onClick={() => handleWordSelect(w)} label={`${w}`} />)}
          </>}
      </div>
    </div>;
}
function OptBtn({ active, onClick, label }) {
  return <button
    onClick={onClick}
...
function ClockIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="mr-1.5">
