...
function KeyboardShortcuts({ isOpen, onClose }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#323437] bg-opacity-90 animate-fade-in-up">
  ...
  >
              <div className="flex items-center gap-2">
                {shortcut.keys.map((key, keyIndex) => <span key={keyIndex} className="flex items-center gap-2">
                    {key !== "+" ? <kbd className="px-3 py-2 bg-[#2c2e31] border border-[#646669] border-opacity-30 rounded text-[#d1d0c5] font-mono-swift text-sm">
                        {key}
                      </kbd> : <span className="text-[#646669] text-sm">+</span>}
                  </span>)}
              </div>
