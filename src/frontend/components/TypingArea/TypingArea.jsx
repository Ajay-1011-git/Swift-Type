...
function TypingArea() {
  ...
  const handleKey = useCallback(
    (e) => {
      ...
      if (e.key !== "Backspace" && e.key !== " " && (e.key.length !== 1 || e.ctrlKey || e.altKey || e.metaKey)) {
        return;
      }
  ...
  return <div className="w-full max-w-[850px] mx-auto relative">
      {
    /* Timer / WPM display */
  }
      {status === "running" && <div className="flex items-center justify-between mb-6">
  ...
  >
    ...
    >
      ...
      return <span key={charIdx} className="relative inline-block">
                        {isCursor && <span
      ...
      return <span key={`extra-${extraIdx}`} className={getCharColor(state)}>
                        {extraChar}
                      </span>;
    })}
      ...
      /* Cursor at end of word */
                  ...
                  {isActive && currentCharIndex >= word.length && status !== "finished" && <span className="relative inline-block w-0">
                      <span
      className="absolute left-0 top-[4px] w-[2px] bg-[#e2b714] rounded-full caret-blink z-10"
      style={{ height: "calc(100% - 8px)", transform: "translateX(-1px)" }}
    />
  ...
  >
    ...
    /* Keyboard hints */
      ...
      {status !== "idle" && <div className="mt-2 text-center">
